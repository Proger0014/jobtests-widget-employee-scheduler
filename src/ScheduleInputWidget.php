...
class ScheduleInputWidget extends InputWidget
{
    ...
    private function renderIconButton($icon, string $buttonType, bool $withNone = true, bool $withMr = true): string {
        $class = 'btn btn-icon';
    ...
    private function renderTimeInput(string $name): string {
        $render = Html::beginTag('div', ['class' => 'form-group', 'widget-input-container' => true]);
    ...
    private function renderDayInput(string $content, int $index, array $binds): string {
        $render = Html::beginTag('label', ['class' => 'day form-group', 'widget-input-container' => true]);
    ...
    public function checkRequirementsModel(): void {
        ...
        if (!$model instanceof ScheduleModel && !is_subclass_of($model, ScheduleModel::class, false)) {
